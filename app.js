...
function productFromRemote(row){
  ...
  return normaliseProduct({id:row.id,codigo:row.code,nombre:row.name,categoria:row.category?.name||'Sin categoría',categoria_id:row.category?.id??null,cantidad_bulto:row.bulk_quantity,observaciones:row.notes,active:row.active,...Object.fromEntries(PRICE_FIELDS.map(field=>[field.key,prices[field.tier]??null]))});
}
...
function bindStatic(){
  ...
  $('#new-product').onclick=openNewProduct;$('#download-excel').onclick=()=>downloadExcel(filtered());$('#print-price-list').onclick=printPriceList;$('#share-price-list').onclick=sharePriceList;$('#manage-categories').onclick=openCategoryManager;$('#save-backup').onclick=saveBackup;$('#view-backups').onclick=openBackups;$('#open-history').onclick=openHistory;$('#open-increase').onclick=()=>openIncrease(false);$('#selection-increase').onclick=()=>openIncrease(true);$('#selection-quote').onclick=()=>openQuote(true);$('#clear-selection').onclick=()=>{state.selected.clear();renderCatalog();};
...
function renderCatalog(){
  ...
  $('#catalog').innerHTML=categories.map(category=>{const products=items.filter(product=>product.categoria===category),categoryId=products[0]?.categoria_id??'';return `<section class="category-block"><div class="category-heading"><label class="category-select"><input class="category-check" type="checkbox" data-category="${esc(category)}" aria-label="Seleccionar categoría ${esc(category)}"><strong>${esc(category)}</strong></label><div class="category-tools"><span>${number(products.length)}</span><button type="button" data-edit-category-id="${categoryId}" data-edit-category-name="${esc(category)}" aria-label="Editar categoría ${esc(category)}">Editar nombre</button></div></div><div class="price-columns-header"><span></span><span>Producto</span>${PRICE_FIELDS.map(field=>`<span>${field.label}</span>`).join('')}<span>Contenido del bulto</span></div>${products.map(productRow).join('')}</section>`;}).join('');updateSelectionBar();syncCategoryChecks();
}

function productRow(product){
  return `<div class="product-row"><input class="product-check" type="checkbox" data-id="${product.id}" ${state.selected.has(product.id)?'checked':''} aria-label="Seleccionar ${esc(product.nombre)}"><div class="product-info"><strong>${esc(product.nombre)}</strong>${product.observaciones?`<small><span class="product-note">${esc(product.observaciones)}</span></small>`:''}</div>${PRICE_FIELDS.map(field=>`<div class="price-wrap"><span class="mobile-price-label">${field.label} ($)</span><input class="price-input" data-id="${product.id}" data-tier="${field.tier}" value="${priceInput(product[field.key])}" inputmode="numeric" placeholder="—" aria-label="Precio ${field.label} de ${esc(product.nombre)}"></div>`).join('')}<div class="bulk-quantity"><span class="mobile-price-label">Contenido del bulto</span><span>${esc(product.cantidad_bulto||'—')}</span></div></div>`;
}
...
function toggleCategorySelection(input){const products=state.products.filter(product=>product.categoria===input.dataset.category);for(const product of products)input.checked?state.selected.add(product.id):state.selected.delete(product.id);renderCatalog();}
function syncCategoryChecks(){document.querySelectorAll('.category-check').forEach(input=>{const products=state.products.filter(product=>product.categoria===input.dataset.category),selected=products.filter(product=>state.selected.has(product.id)).length;input.checked=products.length>0&&selected===products.length;input.indeterminate=selected>0&&selected<products.length;});}
function categoryRecords(){const records=new Map();for(const product of state.products){if(!records.has(product.categoria))records.set(product.categoria,{id:product.categoria_id,name:product.categoria,count:0});records.get(product.categoria).count++;}return [...records.values()].sort((a,b)=>alphabetical(a.name,b.name));}
...
function quoteData(){syncQuoteFields();const rows=quoteRows(),totals=quoteTotals(rows);return {number:state.quote.number,client:state.quote.client,address:state.quote.address,date:prettyDate(),items:rows.map(row=>({name:row.product.nombre,category:row.product.categoria,priceLabel:row.field.label+(row.field.tier==='bulto'&&row.product.cantidad_bulto?` (${row.product.cantidad_bulto})`:''),quantity:row.quantity,unitPrice:row.unitPrice,amount:row.amount})),...totals};}
