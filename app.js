...
function bindStatic(){
  ...
  $('#new-product').onclick=openNewProduct;$('#download-excel').onclick=()=>downloadExcel(filtered());$('#print-price-list').onclick=printPriceList;$('#share-price-list').onclick=sharePriceList;$('#save-backup').onclick=saveBackup;$('#view-backups').onclick=openBackups;$('#open-history').onclick=openHistory;$('#open-increase').onclick=()=>openIncrease(false);$('#selection-increase').onclick=()=>openIncrease(true);$('#selection-quote').onclick=()=>openQuote(true);$('#clear-selection').onclick=()=>{state.selected.clear();renderCatalog();};
...
function openNewProduct(){
  const categories=unique('categoria');
...
function updateIncreasePreview(){
  const changes=increaseChanges(),products=new Set(changes.map(change=>change.product.id)).size,tiers=PRICE_FIELDS.filter(field=>state.increase.tiers.has(field.tier)).map(field=>field.label),percentage=Number(state.increase.percentage),valid=changes.length&&tiers.length&&Number.isFinite(percentage)&&percentage>-100;
  $('#increase-products').textContent=number(products);$('#increase-prices').textContent=number(changes.length);$('#increase-tiers').textContent=number(tiers.length);$('#increase-notice').innerHTML=`<strong>Se modificarán:</strong> ${esc(tiers.join(', ')||'ninguna presentación')}. El cambio se aplica al confirmar.`;const button=$('#increase-confirm');button.disabled=!valid;button.textContent=percentage<0?'Aplicar descuento':'Aplicar aumento';
}
...
function generateQuoteNumber(){const now=new Date(),parts=new Intl.DateTimeFormat('en-CA',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'America/Argentina/Buenos_Aires'}).formatToParts(now).reduce((all,part)=>(all[part.type]=part.value,all),{});return `PR-${parts.year}${parts.month}${parts.day}-${parts.hour}${parts.minute}${parts.second}`;}
function ensureQuoteNumber(){if(!state.quote.number)state.quote.number=generateQuoteNumber();return state.quote.number;}
...
function quoteData(assignNumber=false){syncQuoteFields();const rows=quoteRows(),totals=quoteTotals(rows);return {number:assignNumber?ensureQuoteNumber():state.quote.number,client:state.quote.client,address:state.quote.address,date:prettyDate(),items:rows.map(row=>({name:row.product.nombre,category:row.product.categoria,priceLabel:row.field.label+(row.field.tier==='bulto'&&row.product.cantidad_bulto?` (${row.product.cantidad_bulto})`:''),quantity:row.quantity,unitPrice:row.unitPrice,amount:row.amount})),...totals};}
...
async function printQuotePdf(){const data=quoteData(true);if(!data.items.length)return showToast('Agregá al menos un producto.');const target=window.open('','_blank');try{const bytes=await window.RomaDocuments.buildQuotePdf(data);window.RomaDocuments.print(bytes,target);showToast(`Presupuesto ${data.number} listo para imprimir o guardar como PDF.`);}catch(error){target?.close();showToast(readableError(error));}}
async function shareQuotePdf(){const data=quoteData(true);if(!data.items.length)return showToast('Agregá al menos un producto.');const bytes=await window.RomaDocuments.buildQuotePdf(data),result=await window.RomaDocuments.sharePdf(bytes,`papelera-roma-presupuesto-${safeName(data.client||data.number)}-${todayIso()}.pdf`,'Presupuesto Papelera Roma');if(result.downloaded){window.open(`https://wa.me/?text=${encodeURIComponent(`Presupuesto ${data.number} de Papelera Roma. Adjuntá el PDF descargado.`)}`,'_blank','noopener');showToast('PDF descargado para adjuntar en WhatsApp.');}}
function shareQuoteText(){const data=quoteData(true);if(!data.items.length)return showToast('Agregá al menos un producto.');const lines=[`*PAPELERA ROMA · PRESUPUESTO ${data.number}*`,`Fecha: ${data.date}`,data.client?`Cliente: ${data.client}`:'','',...data.items.map(item=>`${item.quantity} × ${item.name} · ${item.priceLabel} — ${money(item.amount)}`),'',`Subtotal: ${money(data.subtotal)}`,data.discountPercentage?`Descuento ${data.discountPercentage}%: - ${money(data.discountAmount)}`:'',`*TOTAL: ${money(data.total)}*`].filter(Boolean);window.open(`https://wa.me/?text=${encodeURIComponent(lines.join('\n'))}`,'_blank','noopener');}

function priceListData(){return {items:filtered(),title:state.filters.category||'Lista completa',date:prettyDate(),priceFields:PRICE_FIELDS};}
...
async function sharePriceList(){const data=priceListData();if(!data.items.length)return showToast('No hay productos para compartir.');const bytes=await window.RomaDocuments.buildPriceListPdf(data),filename=`papelera-roma-lista-precios-${todayIso()}.pdf`,result=await window.RomaDocuments.sharePdf(bytes,filename,'Lista de precios Papelera Roma');if(result.downloaded){window.open(`https://wa.me/?text=${encodeURIComponent('Lista de precios de Papelera Roma. Adjuntá el PDF descargado.')}`,'_blank','noopener');showToast('Lista descargada para adjuntar en WhatsApp.');}}
...
function downloadQuoteExcel(){const data=quoteData(true);if(!data.items.length)return showToast('Agregá al menos un producto.');const blob=new Blob([quoteExcelDocument(data)],{type:'application/vnd.ms-excel;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`papelera-roma-presupuesto-${safeName(data.client||data.number)}-${todayIso()}.xls`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);showToast(`Presupuesto ${data.number} exportado a Excel.`);}
...
async function refreshBackups(){if(state.preview)return;state.backups=await db('catalog_backups?select=id,label,product_count,price_count,created_at&order=created_at.desc&limit=20');renderSummary();}
