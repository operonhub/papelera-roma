...
const catalogSuffix=catalog=>catalog==='heladeria'?'H':'P';
...
const validProductCode=(code,catalog)=>new RegExp(`^[0-9]{5}-${catalogSuffix(catalog)}$`).test(String(code).trim().toUpperCase())&&Number(String(code).slice(0,5))>0;
function nextProductCode(catalog=state.catalog){const suffix=catalogSuffix(catalog),highest=state.products.filter(product=>product.catalogo===catalog&&new RegExp(`^[0-9]{5}-${suffix}$`).test(product.codigo)).reduce((max,product)=>Math.max(max,Number(product.codigo.slice(0,5))||0),0);return `${String(highest+1).padStart(5,'0')}-${suffix}`;}
...
function bindStatic(){
  ...
  $('#new-product').onclick=openNewProduct;$('#download-excel').onclick=()=>downloadExcel(filtered());$('#print-price-list').onclick=openPrintPriceList;$('#share-price-list').onclick=sharePriceList;$('#manage-categories').onclick=openCategoryManager;$('#save-backup').onclick=saveBackup;$('#view-backups').onclick=openBackups;$('#open-increase').onclick=()=>openIncrease(false);$('#selection-increase').onclick=()=>openIncrease(true);$('#selection-quote').onclick=()=>openQuote(true);$('#clear-selection').onclick=clearSelection;
...
async function createProduct(event){
  ...
  if(code&&!validProductCode(code,state.catalog)){error.textContent=`El código debe tener el formato ${state.catalog==='heladeria'?'00001-H':'00001-P'}.`;error.hidden=false;return;}
...
async function updateProduct(event,product){
  ...
  if(!validProductCode(code,product.catalogo)){error.textContent=`El código debe tener el formato ${product.catalogo==='heladeria'?'00001-H':'00001-P'}.`;error.hidden=false;button.disabled=false;button.textContent='Guardar cambios';return;}
  try{
