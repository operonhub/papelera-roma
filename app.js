...
async function loadCloudData(){
  ...
  const [rows,backups,history]=await Promise.all([
    dbPages('products?select=id,code,name,bulk_quantity,notes,active,catalog:catalogs!inner(slug,name),category:categories(id,name),prices:product_prices(tier,amount)&active=eq.true&order=name.asc'),
...
function setCatalog(slug){if(!CATALOGS[slug]||slug===state.catalog)return;state.catalog=slug;state.filters.search='';state.filters.category='';state.selected.clear();$('#search').value='';document.querySelectorAll('[data-catalog]').forEach(button=>button.classList.toggle('active',button.dataset.catalog===slug));renderAll();}
...
function totalPriceCells(products=catalogProducts()){return products.reduce((sum,product)=>sum+PRICE_FIELDS.filter(field=>isPrice(product[field.key])).length,0);}
...
async function createProduct(event){
  ...
  try{
    if(state.preview){state.products.push(normaliseProduct({id:crypto.randomUUID(),name,categoria:category,catalog_slug:state.catalog,bulk_quantity:$('#product-bulk').value,notes:$('#product-notes').value,...Object.fromEntries(PRICE_FIELDS.map(field=>[field.key,prices[field.tier]??null]))}));}
    else{await rpc('papelera_create_product',{p_name:name,p_category:category,p_bulk_quantity:$('#product-bulk').value.trim(),p_notes:$('#product-notes').value.trim(),p_prices:prices,p_catalog_slug:state.catalog});await loadCloudData();}
...
function quoteData(){syncQuoteFields();const rows=quoteRows(),totals=quoteTotals(rows);return {number:state.quote.number,client:state.quote.client,address:state.quote.address,date:prettyDate(),items:rows.map(row=>({name:row.product.nombre,category:`${CATALOGS[row.product.catalogo]?.name||row.product.catalogo} · ${row.product.categoria}`,priceLabel:row.field.label+(row.field.tier==='bulto'&&row.product.cantidad_bulto?` (${row.product.cantidad_bulto})`:''),quantity:row.quantity,unitPrice:row.unitPrice,amount:row.amount})),...totals};}
...
function priceListData(){return {items:filtered(),title:`${catalogName()} · ${state.filters.category||'Lista completa'}`,date:prettyDate(),priceFields:PRICE_FIELDS};}
