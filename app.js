...
function productRow(product){
  ...
  const code=state.filters.search.trim()?`<span class="product-code">${esc(product.codigo)}</span>`:'';
  return `<div class="product-row"><input class="product-check" type="checkbox" data-id="${product.id}" ${state.selected.has(product.id)?'checked':''} aria-label="Seleccionar ${esc(product.nombre)}"><div class="product-info"><strong>${esc(product.nombre)}</strong>${code||product.observaciones?`<small>${code}${product.observaciones?`<span class="product-note">${esc(product.observaciones)}</span>`:''}</small>`:''}</div>${detail}${actions}</div>`;
}
