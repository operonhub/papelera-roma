-- Heladeria usa las columnas Producto, Cantidad/Presentacion y Precio.

alter table public.products
  add column if not exists presentation text not null default '';

comment on column public.products.presentation is
  'Cantidad o presentacion comercial de listas con un unico precio, como Heladeria.';

update public.products p
set presentation=p.bulk_quantity,
    bulk_quantity=''
where p.catalog_id=(select id from public.catalogs where slug='heladeria')
  and p.presentation='';

alter table public.product_prices drop constraint if exists product_prices_tier_check;
alter table public.product_prices add constraint product_prices_tier_check
  check (tier in ('unidad','x10','x50','x100','bulto','precio'));

alter table public.price_changes drop constraint if exists price_changes_tier_check;
alter table public.price_changes add constraint price_changes_tier_check
  check (tier in ('unidad','x10','x50','x100','bulto','precio'));

alter table public.quote_items drop constraint if exists quote_items_price_tier_check;
alter table public.quote_items add constraint quote_items_price_tier_check
  check (price_tier in ('unidad','x10','x50','x100','bulto','precio'));

create or replace function public.papelera_set_product_price(p_product_id uuid,p_tier text,p_amount numeric)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare v_old numeric(14,2);v_batch uuid;
begin
  if not public.is_app_user() then raise exception 'Acceso denegado';end if;
  if p_tier not in ('unidad','x10','x50','x100','bulto','precio') then raise exception 'Presentacion invalida';end if;
  if p_amount is not null and p_amount<0 then raise exception 'Precio invalido';end if;
  select amount into v_old from public.product_prices where product_id=p_product_id and tier=p_tier;
  insert into public.price_change_batches(change_type,scope_type,scope_label,affected_products,affected_prices,created_by)
  values('manual','product',(select name from public.products where id=p_product_id),1,1,auth.uid()) returning id into v_batch;
  if p_amount is null then delete from public.product_prices where product_id=p_product_id and tier=p_tier;
  else insert into public.product_prices(product_id,tier,amount,updated_by) values(p_product_id,p_tier,round(p_amount,2),auth.uid()) on conflict(product_id,tier) do update set amount=excluded.amount,updated_by=excluded.updated_by;
  end if;
  insert into public.price_changes(batch_id,product_id,tier,old_amount,new_amount,created_by) values(v_batch,p_product_id,p_tier,v_old,p_amount,auth.uid());
  return jsonb_build_object('old_amount',v_old,'new_amount',p_amount,'batch_id',v_batch);
end;$$;

drop function if exists public.papelera_create_product(text,text,text,text,jsonb,text);
create function public.papelera_create_product(p_name text,p_category text,p_bulk_quantity text default '',p_presentation text default '',p_notes text default '',p_prices jsonb default '{}'::jsonb,p_catalog_slug text default 'papelera')
...
begin
  ...
  insert into public.categories(catalog_id,name,active) values(v_catalog_id,btrim(p_category),true) on conflict(catalog_id,name) do update set active=true returning id into v_category_id;
  v_code:=upper(left(p_catalog_slug,3))||'-'||upper(replace(v_product_id::text,'-',''));
  insert into public.products(id,code,name,catalog_id,category_id,bulk_quantity,presentation,notes,active)
  values(v_product_id,v_code,btrim(p_name),v_catalog_id,v_category_id,coalesce(p_bulk_quantity,''),coalesce(p_presentation,''),coalesce(p_notes,''),true);
  for v_price in select key,value from jsonb_each_text(coalesce(p_prices,'{}'::jsonb)) loop
    if v_price.key not in ('unidad','x10','x50','x100','bulto','precio') then raise exception 'Presentacion invalida';end if;
...
create or replace function public.papelera_apply_price_increase(p_percentage numeric,p_tiers text[],p_scope text,p_category text default null,p_product_ids uuid[] default null,p_catalog_slug text default 'papelera')
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
  ...
  if coalesce(array_length(p_tiers,1),0)=0 or exists(select 1 from unnest(p_tiers) tier where tier not in ('unidad','x10','x50','x100','bulto','precio')) then raise exception 'Presentaciones invalidas';end if;
...
create or replace function public.papelera_create_catalog_backup(p_label text default '',p_catalog_slug text default 'papelera')
returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
  ...
  insert into public.catalog_backups(catalog_id,label,product_count,price_count,snapshot,created_by) values(v_catalog_id,coalesce(nullif(btrim(p_label),''),'Copia del catalogo'),v_product_count,v_price_count,jsonb_build_object('version',3,'catalog',p_catalog_slug,'products',v_products),auth.uid()) returning id into v_id;return v_id;
end;$$;
...
begin
  ...
  for v_item in select value from jsonb_array_elements(v_snapshot->'products') loop
    ...
    insert into public.products(id,code,name,catalog_id,category_id,bulk_quantity,presentation,notes,source_row,active) values((v_item->>'id')::uuid,v_item->>'code',v_item->>'name',v_catalog_id,v_category_id,coalesce(v_item->>'bulk_quantity',''),coalesce(v_item->>'presentation',''),coalesce(v_item->>'notes',''),nullif(v_item->>'source_row','')::integer,coalesce((v_item->>'active')::boolean,true)) on conflict(code) do update set name=excluded.name,catalog_id=excluded.catalog_id,category_id=excluded.category_id,bulk_quantity=excluded.bulk_quantity,presentation=excluded.presentation,notes=excluded.notes,source_row=excluded.source_row,active=excluded.active returning id into v_product_id;
