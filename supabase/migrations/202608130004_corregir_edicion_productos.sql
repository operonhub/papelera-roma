-- Ajusta las funciones a la estructura real de products, sin updated_by.
...
begin
  ...
  select catalog_id into v_catalog_id from public.products where id=p_product_id and active=true;
  ...
  set name=btrim(p_name),category_id=v_category_id,
      bulk_quantity=coalesce(p_bulk_quantity,''),presentation=coalesce(p_presentation,''),
...
begin
  ...
  where id=p_product_id and active=true returning name into v_name;
