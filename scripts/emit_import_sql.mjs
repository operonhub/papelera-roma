...
const priceColumns=catalog==='heladeria'?{precio:'precio'}:{unidad:'precio_unidad',x10:'precio_10',x50:'precio_50',x100:'precio_100',bulto:'precio_bulto'};
const productRows=rows.map(r=>({code:r.codigo,name:r.nombre,category:r.categoria,supplier:r.proveedor||null,bulk_quantity:r.cantidad_bulto||'',presentation:r.presentacion||'',notes:r.observaciones||'',source_row:r.fuente_fila?Number(r.fuente_fila):null,active:String(r.activo).toUpperCase()!=='FALSE'}));
...
insert into public.suppliers(name) select name from jsonb_to_recordset($payload$${json(suppliers)}$payload$::jsonb) as x(name text) on conflict(name) do update set active=true;`);
  ...
  process.stdout.write(`with catalog as (select id from public.catalogs where slug='${catalog}'), src as (select * from jsonb_to_recordset($payload$${json(part)}$payload$::jsonb) as x(code text,name text,category text,supplier text,bulk_quantity text,presentation text,notes text,source_row integer,active boolean)), resolved as (select s.*,catalog.id catalog_id,c.id category_id,p.id supplier_id from src s cross join catalog join public.categories c on c.catalog_id=catalog.id and c.name=s.category left join public.suppliers p on p.name=s.supplier) insert into public.products(code,name,catalog_id,category_id,supplier_id,bulk_quantity,presentation,notes,source_row,active) select code,name,catalog_id,category_id,supplier_id,bulk_quantity,presentation,notes,source_row,active from resolved on conflict(code) do update set name=excluded.name,catalog_id=excluded.catalog_id,category_id=excluded.category_id,supplier_id=excluded.supplier_id,bulk_quantity=excluded.bulk_quantity,presentation=excluded.presentation,notes=excluded.notes,source_row=excluded.source_row,active=excluded.active;`);
