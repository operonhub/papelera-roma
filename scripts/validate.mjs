...
for(const file of ['index.html','styles.css','app.js','documentos.js','assets/logo-papelera-roma.png','assets/logo-papelera-roma-pdf.jpg'])await fs.access(file);
...
for(const id of ['catalog-tab','quote-tab','open-increase','save-backup','download-excel','print-price-list','share-price-list','selection-quote','quote-discount','quote-subtotal','quote-total'])if(!index.includes(`id="${id}"`))throw Error(`Falta el control ${id}`);
...
for(const fn of ['openIncrease','openQuote','downloadQuoteExcel','printQuotePdf','shareQuoteText','printPriceList','sharePriceList','saveBackup'])if(!app.includes(`function ${fn}`))throw Error(`Falta la función ${fn}`);
