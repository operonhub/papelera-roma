...
for(const expected of ['rel="manifest" href="manifest.webmanifest"','rel="apple-touch-icon" href="assets/icon-180.png"'])if(!index.includes(expected))throw Error(`Falta la configuración instalable: ${expected}`);
if(manifest.name!=='Papelera Roma · Gestión comercial'||manifest.short_name!=='Papelera Roma'||manifest.display!=='standalone'||manifest.start_url!=='/'||manifest.icons?.length!==2)throw Error('El manifest instalable no coincide');
for(const [file,size] of [['assets/icon-180.png',180],['assets/icon-192.png',192],['assets/icon-512.png',512]]){const png=await fs.readFile(file);if(png.readUInt32BE(16)!==size||png.readUInt32BE(20)!==size)throw Error(`${file} debe medir ${size}x${size}`);}
