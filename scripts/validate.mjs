...
const heladeriaWorkbook=await Workbook.fromCSV(heladeriaText,{sheetName:'Heladería'}),heladeriaRows=heladeriaWorkbook.worksheets.getItem('Heladería').getRange('A1:M88').values,heladeriaProducts=heladeriaRows.slice(1).filter(row=>row[0]);
if(heladeriaProducts.length!==87)throw Error(`Se esperaban 87 productos de Heladería y hay ${heladeriaProducts.length}`);
if(new Set(heladeriaProducts.map(row=>row[2])).size!==17)throw Error('Las categorías de Heladería no coinciden');
const heladeriaPrices=heladeriaProducts.map(row=>row[8]).filter(value=>value!==null&&value!==undefined&&String(value).trim()!=='').map(Number);
if(heladeriaPrices.length!==41||heladeriaPrices.reduce((sum,value)=>sum+value,0)!==1092505)throw Error('Los precios de Heladería no coinciden');
for(const file of ['index.html','styles.css','app.js','documentos.js','data/productos_heladeria.csv','assets/logo-papelera-roma.png','assets/logo-papelera-roma-pdf.jpg','supabase/migrations/202608120009_quote_numbers_and_category_editing.sql','supabase/migrations/202608120010_harden_quote_numbers_and_category_editing.sql','supabase/migrations/202608130001_catalogos_separados.sql'])await fs.access(file);
...
for(const fn of ['setCatalog','catalogProducts','openIncrease','openQuote','openCategoryManager','renameCategory','nextQuoteNumber','issuedQuoteData','downloadQuoteExcel','printQuotePdf','shareQuoteText','printPriceList','sharePriceList','saveBackup'])if(!app.includes(`function ${fn}`))throw Error(`Falta la función ${fn}`);
